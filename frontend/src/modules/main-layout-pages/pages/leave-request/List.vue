<template>
  ...
  <b-card class="mt-3">
    <b-card-title>
        <b-row>
          ...
          <b-col class="text-right">
            <b-button v-if="userRole == 'employee'" size="sm" variant="primary" @click="openAddNewModal()"><i class="ri-add-fill"></i>Add New Leave</b-button>
          </b-col>
        </b-row>
      </b-card-title>
      <b-row>
                    ...
                    <tbody v-for="(item, index) in listData" :key="index">
                      <tr style="font-size: 12px;">
                        ...
                        <td class="text-center">
                          <a v-tooltip="'Edit'" v-if="userRole == 'employee'" style="width: 20px !important; height: 20px !important; font-size:10px" href="javascript:" class="action-btn edit" @click="editData(item)"><i class="ri-pencil-fill"></i></a>
                          <a v-tooltip="'Change Status'" v-else style="width: 20px !important; height: 20px !important; font-size:10px" href="javascript:" class="action-btn edit" @click="changeStatus(item)"><i class="ri-pencil-fill"></i></a>
    ...
    <b-modal id="modal-1" ref="editModal" size="md" title="Leave Request Form" :hide-footer="true">
      <Form @loadList="loadData" :editItem="editItem"/>
    </b-modal>

    <b-modal id="modal-1" ref="changeStatusModal" size="md" title="Change Status" :hide-footer="true">
      <ChangeStatus @loadList="loadData" :editItem="editItem"/>
    </b-modal>
    <!-- pagination -->
...
export default {
  ...
  computed: {
    ...
    userRole: function () {
      return this.cn(this.$store.state, 'Auth.authUser.role')
    }
  },
  methods: {
    openAddNewModal () {
    ...
    async loadData () {
      ...
      var result = await RestApi.getData(baseURL, 'api/leave-request/getAllLeaveRequest')
