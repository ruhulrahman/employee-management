...
export default {
  ...
  methods: {
    async submitData () {
      ...
      let result = ''
      if (this.form.id) {
        result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/update_channel_data', this.form)
      } else {
        result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/store_channel_data', this.form)
      }
      this.loading = false
      if (result.success) {
    ...
    async getChannelLastSerial () {
      const result = await RestApi.getData(baseURL, 'api/v1/admin/ajax/get_channel_last_serial')
      if (result.success) {
