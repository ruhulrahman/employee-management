<template>
  <div class="section-wrapper">
    ...
    <b-breadcrumb>
      ...
      <b-breadcrumb-item active>Service Status</b-breadcrumb-item>
    </b-breadcrumb>
      <div class="form-wrapper">
      <b-card title="Service Status Search">
          <b-card-text>
  ...
  <b-card class="mt-3">
    <b-card-title>
      <b-row>
        <b-col>
          <h4 class="card-title mb-0 pl-0">Service Status List</h4>
        </b-col>
              ...
              <table class="table table-striped table-hover table-bordered">
                ...
                <tbody v-for="(item, index) in listData" :key="index">
                  <tr style="font-size: 12px;">
                    ...
                    <td class="text-left">
                      <span class="badge badge-pill" :style="`background: ${item.color_code}; color: white;`">{{ item.name }}</span>
                    </td>
                    ...
                    <td class="text-center">
                      <span v-html="item.color_code"></span>
                    </td>
                    <td class="text-center">
    ...
    <b-modal id="modal-1" ref="editModal" size="lg" title="Service Status" centered :hide-footer="true">
...
export default {
  ...
  methods: {
    ...
    async getListData () {
      ...
      var result = await RestApi.getData(baseURL, 'api/v1/admin/ajax/get_service_status_list', params)
      if (result.success) {
    ...
    async toggleActiveStatus (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/toggle_service_status_active_status', item)
      this.loading = false
      if (result.success) {
    ...
    async deleteConfirmed (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/delete_service_status_data', item)
      if (result.success) {
        this.$toast.success({
          title: 'Success',
