<template>
  <div class="section-wrapper">
    ...
    <b-breadcrumb>
      ...
      <b-breadcrumb-item active>Course Level</b-breadcrumb-item>
    </b-breadcrumb>
      <div class="form-wrapper">
      <b-card title="Course Level Search">
          <b-card-text>
  ...
  <b-card class="mt-3">
    <b-card-title>
      <b-row>
        <b-col>
          <h4 class="card-title mb-0 pl-0">Course Level List</h4>
        </b-col>
    ...
    <b-modal id="modal-1" ref="editModal" size="lg" title="Course Level" centered :hide-footer="true">
...
export default {
  ...
  methods: {
    ...
    async getListData () {
      ...
      var result = await RestApi.getData(baseURL, 'api/v1/admin/ajax/get_course_level_list', params)
      if (result.success) {
    ...
    async toggleActiveStatus (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/toggle_course_level_active_status', item)
      this.loading = false
      if (result.success) {
    ...
    async deleteConfirmed (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/delete_course_level_data', item)
      this.loading = false
      if (result.success) {
        this.$toast.success({
          title: 'Success',
    ...
    async serialUp (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/course_level_serial_up', item)
      this.loading = false
      if (result.success) {
        this.$toast.success({
          title: 'Success',
    ...
    async serialDown (item) {
      this.loading = true
      var result = await RestApi.postData(baseURL, 'api/v1/admin/ajax/course_level_serial_down', item)
      this.loading = false
      if (result.success) {
        this.$toast.success({
          title: 'Success',
